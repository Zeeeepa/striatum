...
What happened? Include the command, work packet, source, or prompt that made
...
What should change in the runner, workflow contract, prompts, defaults, tool
profile fixture, spec, or docs? Keep the front-matter `target` value inside
the schema's allowed set: `prompt`, `workflow`, `spec`, `defaults`, or
`documentation`.
