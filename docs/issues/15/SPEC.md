...
- `README.md` still says `.striatum/state.sqlite3` is authoritative live state and reports an old status/version.
- `docs/SPEC.md` still says SQLite under `.striatum/state.sqlite3` is authoritative, even though D094/RFC0043 move workflow state to daemon-owned PostgreSQL.
- `docs/GETTING_STARTED.md` omits PostgreSQL from prerequisites and the quick-start path.
- `docs/HOW_TO_HUMAN.md` still says `init` creates `.striatum/state.sqlite3` and describes RFC0033 as leaving repo-local SQLite as live run state.
...
- Skill/plugin templates still tell agents that `.striatum/state.sqlite3` is live state / the substrate not to touch.
...
- No current product doc claims `.striatum/state.sqlite3` is authoritative live state except when describing historical V1 behavior, migration fixtures, or tombstone inspection.
