...
Striatum-side documentation consistency pass plus the RFC 0052 scaffold for
...
- **REVIEW F1 (HIGH) — untracked files.** `git add docs/issues/17/
  docs/rfcs/0052-corpus-contract-v2.md` was run, so the RFC 0052 scaffold
  ...
  `git status --short docs/issues/17/ docs/rfcs/0052-corpus-contract-v2.md`
...
- **REVIEW F2 (LOW) — TODO.md top table out of sync.** Row 42 of
  ...
  `⏳ open` to `🟡 docs pass + RFC 0052 scaffold landed; row stays open
  until RFC 0052 V2 acceptance`, matching the detail-section status
...
- Engram is consistently described across Striatum's current product
  ...
  docs.
- A proposed RFC 0052 scaffold lives at
  `docs/rfcs/0052-corpus-contract-v2.md`. It frames the V2 decision
...
- `docs/ENGRAM_INCUBATION_CONTEXT.md` retains its historical status
  ...
  RFC 0044, and RFC 0052 so it cannot mislead a reader who lands
  there first.
- `docs/ROADMAP.md` §5.7 and `docs/TODO.md` item 42 cross-reference
  ...
  explicitly note that GH #17 stays open until RFC 0052 V2 acceptance
  lands.
...
|---|---|---|
| `docs/rfcs/0052-corpus-contract-v2.md` | new | Proposed RFC scaffold for Striatum Corpus Contract V2. 12 numbered decision sections, deferred acceptance criteria, 5 open questions, domain-modeling table with value-object names. Now `git add`ed. |
| `docs/rfcs/README.md` | edit | Added RFC 0052 row to the index (`proposed (scaffold)` status, one-paragraph hook, cross-link to GH #17). |
...
| `docs/CLI_REFERENCE.md` | edit | New "Corpus export" section between Inspection-and-recovery and Adapter. Documents the verb, replay semantics, and cross-link to RFC 0052. |
...
| `docs/HOW_TO_AGENT.md` | edit | New "do not" bullet about not assuming external memory/retrieval availability; packet context + canonical docs remain authoritative; augmentation policy scoped by RFC 0052. |
...
| `docs/ROADMAP.md` | edit | §5.7 gains a "RFC 0052 scaffold landed (2026-05-14)" paragraph linking to the new RFC and noting the deferral of V2 acceptance criteria. |
| `docs/TODO.md` | edit | Detail item 42 already captured the doc-consistency pass status (🟡, list of touched files, "stays open until RFC 0052 V2 acceptance lands"). Attempt 2 also updated the **top status snapshot table** (`docs/TODO.md:100`) so row 42 reads `🟡 docs pass + RFC 0052 scaffold landed; row stays open until RFC 0052 V2 acceptance` instead of `⏳ open`. |
...
- **GH17-1 (single coherent operator path).** SPEC §"Corpus Export And
  ...
  `docs/CLI_REFERENCE.md:326`) ↔ RFC 0052
  (`docs/rfcs/0052-corpus-contract-v2.md`).
...
- **GH17-3 (RFC 0052 scaffold names V2 decisions).** RFC 0052 has 12
...
- **GH17-4 (must run without Engram).** Stated as a non-negotiable
  ...
  Boundary", RFC 0052 Goals + Non-Goals + Decision 10, and
...
- **GH17-6 (Striatum/Engram boundary preserved).** SPEC §"Corpus
  Export And Augmentation Boundary" and RFC 0052 both place
...
- **GH17-7 (narrow PostgreSQL wording).** No new PostgreSQL transition
  ...
  files are in the RFC 0052 Open Question 4 ("how does the Postgres-
...
#       | 🟡 docs pass + RFC 0052 scaffold landed; row stays open until
#         RFC 0052 V2 acceptance |
```
...
- **RFC 0052 scaffold is intentionally non-final.** It frames the
...
- **Doc-consistency, not test-pinned.** The new wording in SPEC, MCP,
  ...
  RFC 0052 names this explicitly).
...
- **Shared parallel-workflow branch.** The worktree includes staged or
  ...
  `git commit -- docs/issues/17/ docs/rfcs/0052-corpus-contract-v2.md
