...
- `docs/rfcs/0052-corpus-contract-v2.md` -- create a proposed RFC scaffold
  ...
  enforcement.
- `docs/rfcs/README.md` -- add RFC 0052 to the index as proposed.
...
- `docs/ROADMAP.md` -- small consistency edits are allowed, especially to
  keep section 5.7 and the GH #17 row aligned after RFC 0052 is scaffolded.
...
- `docs/DECISION_LOG.md`, unless the implementer makes a real product or
  architecture decision beyond a proposed RFC scaffold. A proposed RFC 0052
...
3. GH17-3: A proposed RFC 0052 scaffold exists for Corpus Contract V2 and
...
- RFC numbering conflict: if another parallel workflow creates RFC 0052 first,
...
- Runtime-boundary regression risk: new prose must not suggest that workflow
  ...
  recovery, or run start. Any retrieval/injection policy belongs in RFC 0052
