...
and lives in one of two places inside the target repo, depending
on the profile the operator chose:

- `.claude/skills/striatum-*/SKILL.md` — five files, the
  `claude_code` profile (auto-discovered by Claude Code).
- `striatum-STRIATUM_AGENT_GUIDE.md` — one file at the repo
  root, the `generic` profile (Codex CLI, Gemini CLI, or any
  agent without a built-in skill-discovery convention).

V1 ships only those two profiles. First-class `codex` and
`gemini` profiles are step 3 of RFC 0015 and remain deferred;
until they land, the `generic` guide is the canonical answer for
