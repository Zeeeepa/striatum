...
```bash
export STRIATUM_DAEMON_DB_URL=postgres://striatum@/striatum_daemon
striatum daemon doctor --postgres-url "$STRIATUM_DAEMON_DB_URL" --apply-migrations --json
striatum daemon start --json &
```

`daemon doctor` brings the daemon-owned schema forward; the daemon
process is required before any CLI verb can run. If a verb refuses
with exit code 11 (`daemon_unreachable`), the stderr message names
the socket path and remediation; if it refuses with exit code 12
(`repo_not_migrated`), run the per-repo migration command (see
below and [POSTGRES_TRANSITION.md](POSTGRES_TRANSITION.md)).

If you are upgrading an existing target repo that still has a
populated `.striatum/state.sqlite3`, cut it over once:

```bash
striatum daemon migrate-repo-local \
  --from sqlite --to pg \
  --repo "$TARGET_REPO" --dry-run --json
striatum daemon migrate-repo-local \
  --from sqlite --to pg \
  --repo "$TARGET_REPO" --json
```

The safe default keeps the source as a read-only
`state.sqlite3.tombstone`; pass `--no-keep-sqlite-readonly
--confirm-delete` only when irreversible cleanup is intended.
