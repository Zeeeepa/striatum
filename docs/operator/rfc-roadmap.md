...
| 14b | **0170** | 🛡 | Self-culling repository — the CULL workflow class (detect + shed dead artifacts); the systemic answer to the 2026-06-24 accretion finding. **P0 = `cullable_entity` ledger + read-only Tier-1 (supersession) `DecayTickSweep`, no deletion.** | ⏸ **P0 design cycle-1 = `needs_revision`** (`falsification_gate` `run_85afe0ff`, canceled 2026-06-26). G3 substrate + G4 fwd-compat MET; **G1' + G2' binding** (reconcile the `Status: superseded`-vs-live-cited predicate so `rfc:0097`-class artifacts aren't nominated; add a sub-cadence cull-fold deadline + HANG regression test so a blocked sweep can't stall recovery). v1 dialogue snapshot `/tmp/rfc0170-v1-snapshot/`. **Resume via a fresh `-v2` revising holder** (seed = v1 SPEC + ledger) → D271, then BUILD→VERIFY. **Blocked-by: clear `#616`** (cancel-mid-fanin stranded the barrier; doctor red). | #616 (doctor) | #615 |
