...
| 14b | **0170** | 🛡 | Self-culling repository — the CULL workflow class (detect + shed dead artifacts); the systemic answer to the 2026-06-24 accretion finding. **P0 = `cullable_entity` ledger + read-only Tier-1 (supersession) `DecayTickSweep`, no deletion.** | 🔄 **P0 design v2 active** (`falsification_gate` `run_35064716`, branch `striatum/rfc-0170-p0-design-v2`): holder is revising the cycle-1 SPEC to discharge G1'/G2'. Cycle-1 (`run_85afe0ff`, canceled 2026-06-26) reached `needs_revision`; G3 substrate + G4 fwd-compat MET; **G1' + G2' binding** (reconcile the `Status: superseded`-vs-live-cited predicate so `rfc:0097`-class artifacts aren't nominated; add a sub-cadence cull-fold deadline + HANG regression test so a blocked sweep can't stall recovery). v1 dialogue snapshot `/tmp/rfc0170-v1-snapshot/`. The prior #616 terminal-barrier debris blocker is fixed; a follow-on false-red where dependency-blocked v2 downstream seats looked like hard `BARRIER_BLOCKED` seats is fixed in the current change. Let v2 reach ledger verdict → D271, then BUILD→VERIFY. | — | #615 |
