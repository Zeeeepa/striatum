...
| 2b | **0168** | 🛡 | Per-lane OS uid (pooled) as the lane security principal — dissolves the shared-uid `BC1-W1-ORACLE` wall the 0143 gate hit; **RFC 0143 Slice-B prerequisite** | ✅ **DESIGN ACCEPTED WITH FOLLOW-UP (D272, 2026-06-27)** after v6 convergence. Security-critical findings discharged: scrub classifier, bearer-path re-root, credential-cache ancestry, and fail-closed provider selector completeness. Binding build constraint: add a provider-owned credential-selector discriminator so legitimate non-credential lane env (`AGY_HOME`/`FIXTURE_CONFIG_DIR`) still launches while uncovered provider credential directories resolving in-repo still refuse. v6 run `run_010c81ec8ca17ffd182e0bd7be3f28cc` was canceled after D272 to clear the runner wedge (#620); BUILD is next from the v6 HOLDER spec + D272, with no Claude lanes before 2026-06-30 15:59 UTC unless credentials are restored. | #512 / RFC 0143 Slice B blocker |
...
| 14b | **0170** | 🛡 | Self-culling repository — the CULL workflow class (detect + shed dead artifacts); the systemic answer to the 2026-06-24 accretion finding. **P0 = `cullable_entity` ledger + read-only Tier-1 (supersession) `DecayTickSweep`, no deletion.** | ✅ **P0 design CLEARED → BUILD next** (D271). The `falsification_gate` ran v1→v5 (5 rounds); v5 (`run_76bfeaef`) cleared `accept_with_findings` under a re-scoped P0 bar: G1 = mechanical+sound Tier-1 predicate + known-set corpus (zero false POSITIVES on the known preserved set, `D267` nominated); G2 = read-only-safe cull fold (recovery loop never blocked, cursor refresh not deferred, no torn write, panic-isolated); G3 substrate + G4 fwd-compat intact. Cleared SPEC: `docs/operator/artifacts/rfc-0170-p0-design-v5/commit/proposal/PROPOSAL.md`. P1 deferrals: **#618** (whole-tree `status:frozen` citation exactness — a safe false-negative) + **#619** (non-cooperative FS-hang cull-slot fence). **NEXT: BUILD** — an `rfc-0170-p0-build` `code_change` run implementing the SPEC (migration `0045` + the read-only `DecayTickSweep`), then VERIFY with sealed receipts. | — | #615 |
