...
| Tmux-backed live-interactive supervisor metadata | partial; attach metadata projects through daemon reads and `supervision.require_tmux` fail-closed opt-in landed |
...
| Deadline sweeper and status/dashboard/operator surfaces | partial; protocol liveness and tmux fail-closed opt-in landed; UI polish remains |
