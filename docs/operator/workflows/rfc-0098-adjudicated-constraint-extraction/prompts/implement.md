...
RFC 0098 slice 1 (`collaboration_ledger.v1.1` additive `constraints[]`/
`branches[]` + the productive-refusal publish gate: `needs_revision` ⇒
non-empty `constraints[]`; accept the advertised clearing verbs (#88) and
natural ledger front matter (#79)). Pull in slice 2 (generator shape + fixture,
needs #84 cycle-aware logical names) or slice 3 (discharge-verifying
`final_review`) only if each stays reviewable. Defer slice 4 (first-class
`constraint.*` objects + metrics) to a future run and record the deferral.
