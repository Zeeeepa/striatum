You are the **Committer** for the RFC 0142 P4 design run (**v8 / REVISION**). The
...
Start from the **revised (v8)** Holder's `HOLDER.md` and fold in every challenge the
adjudicator recorded as material-and-incorporated. Because this is the v8 revision
run, the committed spec MUST carry **all eleven** fixes as first-class, build-ready
design — the cycle-1 fix resolved this cycle and the ten earlier fixes carried forward:

- **M6 — the M5 `applied_owner` split propagated coherently through ALL cursor rows**
  (resolved this cycle): the entire 64-cell table is derived mechanically from the two
  predicates W (`CheckOwnerBundleWatermark`) and A (`CheckDeployActivation`); the
  load-bearing invariant is stated explicitly: "once W passes, A is
  ...
  (none, in_progress, finalizing, complete)"; anchored to `schema_drift.go:145-161`
  (`LiveFingerprint` orthogonal to `owner_bundle_meta`) and `schema_drift.go:171-195`
  (`RecordSchemaFingerprint` orthogonal to `owner_bundle_meta`); §3.5 rows 13 and 15 in
  the `==0` column are now conditional — "serve if in-sync, else `awaiting_deploy`" —
  exactly as `==20`; the degenerate 13/`==0`-in-sync idempotent `:399` rewrite is in BOTH
  §4.5 AND the F18 spy list; §4.5 and F18 spy list are consistent (4 cells: 1/`==0`,
  1/`==20`, 13-in-sync/`==0`, 13-in-sync/`==20`); ALL cursor rows × `==0` audited.
- **M5 — the owner-watermark dimension split, row-1** carried forward: `applied_owner`
  ...
  fresh-DB bring-up — `ApplyMigrations` + the legacy `connection.go:399` self-record MAY
  ...
  `awaiting_owner_ddl` (DB untouched) retained for `1 <= applied_owner < 20`; F18
  asserts BOTH the `applied_owner == 0` serve cell AND the `1..19` halt cell; F18a pins
  the fresh-DB serve; cell `==20` relabeled the inert-landing re-boot cell.
...
- **BC-N2 — the universal pre-revoke `deploy_cursor`-authoritative serve edge** carried
  ...
  case; M5/M6 re-bucket the orthogonal owner-watermark dimension at W — neither weakens
  the non-complete `applied_owner == 20` edge.
...
- **C2 — the fail-closed revoke-bundle + `STRIATUM_DEPLOY_DECOUPLED` activation edge**
  ...
  case and M5/M6 re-bucket the owner-watermark dimension without advancing `Required` or
...
with a specified guard/outcome DERIVED MECHANICALLY from W and A, Universal Invariant B
proven in each, §4.5 and F18 spy list consistent, AND the legitimate fresh-DB cells still
serving), and not regress Q4, the per-step Q3-A/Q3-B body, or the shadow-first decoupling
...
- **Resolve Q3 and Q4** with the decided mechanism: the per-step-atomic +
  ...
  the complete-cursor activation guard (M3), the full boot-path decision table with the
  M5-correct owner-watermark dimension split (cell 1/`==0` fresh-DB serve), AND the M6
  mechanical derivation making the decision table coherent across ALL cursor rows (the
  `==0` and `==20` columns having IDENTICAL A-gate outcomes everywhere W passes); and the
...
- **Name the exact surfaces:** the `striatum daemon deploy` command site
  ...
  dimension (cell 1/`==0` fresh-DB serve), AND the M6-coherent full decision table (the
  `==0` column derived from the same W+A invariant as `==20` — anchored to
  `schema_drift.go:145-161` + `schema_drift.go:171-195`); the full-transcript verification
  surface (`go/pkg/db/schema_drift.go` `ExpectedFingerprint`/`LiveFingerprint`/
  ...
  sequenced terminal per C3 so it never locks out the runtime path nor strands the deploy
  nor opens a pre-revoke serve window nor commits early via a self-heal reapply nor is
  reachable via the legacy serve-boot path on a `complete` cursor (M3).
- **Specify the build slices in contract-first order** (smallest safe first), each with
  ...
  the mechanically-derived full 64-cell table (F18 spy list = §4.5 = 4 cells reaching
  `:399`). Be shadow-first.
- **State the explicit Acceptance Criteria** an impl-run + verify-run must meet,
  ...
  `awaiting_owner_ddl` DB-untouched — the F18 dual-cell assertion (M5/row-1); **(g) boot
  a no-revoke binary (decoupled ON) over a `complete`/`applied_owner==0`/in-sync DB and
  prove it SERVES verify-only (NOT `awaiting_deploy`), and separately prove the same binary
  over a `complete`/`applied_owner==0`/out-of-sync DB halts `awaiting_deploy` — the M6
  in-sync `complete`/`==0` assertion (rows 15/`==0`); and (h) boot a no-revoke binary (flag
  OFF) over a `complete`/`applied_owner==0`/in-sync DB and prove it SERVES legacy no-op
  (the idempotent `:399` rewrite) and NOT `awaiting_deploy` — the degenerate 13/`==0`-in-sync
  subcase (F18 spy list)**; (i) prove the deploy receipt is hash-chained and `doctor`
  surfaces `schema_deploy_unrecorded`; (j) in a two-role cluster, run the activation deploy
  through to `complete` and prove runtime-object ownership + post-deploy `striatumd_rw`
  CREATE denial (C3).
