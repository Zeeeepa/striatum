...
## Live verification (DoD #5) — PASSED (2026-05-26), one bug found + fixed

Rebuilt + installed `striatumd` v2.6.0, restarted the user service (schema 17,
no migration). Ran a 2-party conversation: a gemini lane declared
`adapter_capabilities.single_shot: true` driven by `supervise.start`
(`agent_loop_mode=turn_driver`) + a self-driving claude lane. The conversation
(`conv_812fe016…`) reached `closed` at `max_rounds=2`, 4 turns; **gemini's turns
0 and 2 were generated by `striatumd -agent-loop -turn-driver` (no shell
script)**, claude self-drove turns 1 and 3. The exchange was coherent
(validation gating → shared source of truth → append-only ledger →
content-addressed references). F42's core claim is verified end-to-end via the
production path.

**Bug found by live verification (now F44):** the first supervised launches
**zombied immediately** with `agent_loop_mode=turn_driver` but no turns. Root
cause captured by reproducing the exact launch with stderr to a file:
...
The daemon-spawned turn-driver inherits the daemon's systemd `PATH`
(`/usr/local/sbin:…:/snap/bin`), which lacks `~/.local/bin` / `~/.npm-global/bin`
where the `gemini` content-generator binary lives — the recurring
daemon-spawned-lane PATH issue. The driver then *crashes* (RunTurnDriver returns
the error → process exits), and the daemon does not reap it (`cmd.Start` without
`Wait` → zombie) while `supervise.status` still reports stale `alive`.

**Workaround applied (local runtime state, NOT in repo):** a systemd drop-in
`~/.config/systemd/user/striatumd.service.d/path.conf` sets `PATH` to include
`%h/.local/bin:%h/.npm-global/bin`. After restart the supervised turn-driver
found gemini and drove the conversation to close.

**F44 (durable product fix):** (1) `supervise.start` should put the operator's
local bin dirs on the supervised lane `PATH` (or resolve the lane command to an
absolute path) so daemon-spawned single-shot generators are found without
operator systemd surgery; (2) a generator-not-found / repeated generation
failure should **park the floor + escalate**, not crash the whole turn-driver
(the design intended graceful failure but exec-not-found exits the process);
(3) the daemon should reap exited supervised children and not report stale
`alive` liveness.
