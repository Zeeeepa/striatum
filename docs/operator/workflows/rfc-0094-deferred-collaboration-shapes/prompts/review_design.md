...
session. Read
`docs/operator/workflows/rfc-0094-deferred-collaboration-shapes/artifacts/DESIGN_SYNTHESIS.md`. You
...
- `threat_model` — risks the design introduces (especially: does
  emit-before-teardown actually close the liveness race? is the type-sequencing
  gate truly daemon-method-free? is the ledger change additive?).
...
- `devils_advocate` — adversarially probe assumptions, gaps, and contested
  trade-offs (especially the Check-B anti-theater bar).
...
(`docs/operator/workflows/rfc-0094-deferred-collaboration-shapes/artifacts/review/design/<lane>/REVIEW.md`).
