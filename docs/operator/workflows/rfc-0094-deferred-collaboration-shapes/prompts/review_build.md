...
session. Read
`docs/operator/workflows/rfc-0094-deferred-collaboration-shapes/artifacts/build/HANDOFF.md` and the
diff it points to.

Your posture is supplied by the work packet (`review_posture`). Honor it:

- `threat_model` — risks the build introduces (especially: is the ledger
  change truly additive? does the emit-before-teardown ordering hold under a
  dead target? did any new daemon method/route sneak in?).
...
- `devils_advocate` — adversarially probe edge cases, regressions, and
  load-bearing assumptions (run the anti-theater corpus and confirm hollow
  transcripts score `needs_revision`).
...
(`docs/operator/workflows/rfc-0094-deferred-collaboration-shapes/artifacts/review/build/<lane>/REVIEW.md`).
