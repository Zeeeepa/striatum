...
- Decide whether `needs_revision` cycles are productive or whether to
  abort the run and rewrite the synthesis or implementation. When a
  `needs_revision` verdict opens a `revision_routing` human_checkpoint with
  no productive cycle left, record a `striatum.decision` and resolve the
  checkpoint with `checkpoint resolve --action override --decision-id <id>`
  to accept the verdict and proceed (RFC 0094 dogfood exercises #63 F2).
...
`docs/operator/workflows/rfc-0094-deferred-collaboration-shapes/artifacts/`.
