...
title: "RFC 0170 P0 self-culling repository (Tier-1 cullable_entity candidacy substrate + read-only DecayTickSweep) — falsification-cleared consolidated implementation SPEC (v5, re-scoped P0 bar / D271)"
...
# PROPOSAL — RFC 0170 P0: self-culling repository (the Tier-1 `cullable_entity` candidacy substrate + read-only `DecayTickSweep`) — falsification-cleared consolidated implementation SPEC

author: committer-author-001

> **The committed deliverable** of the RFC 0170 P0 design run (v5 — the intended **clearing**
> round under the re-scoped P0 bar). Published **only after** the v5 cycle-1 collaboration ledger
> (`docs/operator/artifacts/rfc-0170-p0-design-v5/dialogue/adjudicator/COLLABORATION_LEDGER_cycle_1.md`)
> **CLEARED the falsification gate** with verdict **`accept_with_findings`** — all four gates
> (**G1** Tier-1 soundness + known-set exactness, **G2** read-only safety, **G3** substrate, **G4**
> forward-compat) **MET**, both falsifier challenges **rebutted** (`landed_and_rebutted`), all four
> posture branches (`tier1_false_positive`, `readonly_safety_latency`, `substrate_correctness`,
> `forward_compatibility`) **cleared**, and both deferrals (**#618**, **#619**) **honestly scoped to
> P1**. This consolidated SPEC is the **single contract** the downstream `rfc-0170-p0-build`
> `code_change` run implements.
>
> **Re-scoped P0 bar (operator decision, ratified as D271).** P0 is **observe-only**: it writes a
> candidacy ledger that **nothing acts on** — no deletion, no tombstone, no page, no `doctor`
> RED/amber, no run-admission effect (§0). The bar therefore pins the **two properties that matter
> for a mirror**: **(G1)** the Tier-1 predicate is mechanical/sound and **never nominates the known
> preserved set** (**zero false POSITIVES** — the dangerous direction), while nominating the
> known-dead `decision:D267`; and **(G2)** the cull fold is **read-only safe**. Exhaustive
> whole-tree Tier-1 exactness (**#618**) and the non-cooperative-FS-hang cull-liveness bound
> (**#619**) are **explicitly P1** — a safe-direction false negative and a restart-recoverable
> liveness gap respectively, **not** P0 blockers. **D271** records this scope boundary in the
> decision log.
>
> **What this proposal is.** The **gate-cleared v5 consolidating-holder SPEC**
> (`docs/operator/artifacts/rfc-0170-p0-design-v5/dialogue/holder/HOLDER.md`,
> `author: holder-author-001`) — which consolidates the whole P0 design across all five rounds
> (`-v1…-v5`) and is reproduced **verbatim** below from §0 onward — published as the committed
> contract, with the adjudicator's **two carried findings folded in (§F)** as accepted P1 deferrals
> **plus** their downstream P0 build-proof obligations. The fold **strengthens** the contract; it
> weakens **no** constraint the adjudicator recorded. Every load-bearing claim in the body is a
> **falsifiable assertion** paired with the **named test/corpus row that would refute it**, anchored
> to a verified `file:line` source site at the run base `striatum/rfc-0170-p0-design-v5`. Scope is
> **P0 only**; the P1+ seams are named (§6) but not designed.
>
> **The contract surface (the build run implements exactly this — every item detailed in the body):**
> (1) the `cullable_entity` **runtime** migration `go/pkg/db/sql/0045_cullable_entity.sql` — `0045`
> free slot; the role-guarded `striatumd_rw` `GRANT SELECT, INSERT, UPDATE` with **no** owner DDL/FK
> for migrations ≥27; **both** the read **and** write authority-inventory rows
> (`readAuthorityInventory["cullable_entity"] = ReadClassRuntimeOperational`,
> `writeAuthorityInventory["cullable_entity"] = ClassRuntimeDML`); the **no-`SELECT *`**
> explicit-column read discipline (the #614 / bundle-0022 SEV-1 lesson) — §2; (2)
> `DecayTickSweep.SweepOnce` in `go/pkg/recovery` — the recovery-tick **piggyback** (no new timer);
> the panic/error-**isolation seam** in the detached goroutine; the **latency safety machinery** —
> `DefaultCullFoldTimeout` over **both** the DB read and the filesystem scan + watchdog +
> single-in-flight **skip-on-overrun** + **L4 compute-then-commit** + the **off-the-`wait`-path fold**
> so the persisted-cursor refresh is **never deferred** — §4; (3) the **Tier-1-ONLY** supersession
> predicate — the **clause-4 active-baseline citation rule** (with the clause-4 `D174` preservation),
> the mechanical **`kind=decision` successor rule** (`D267→{D270}`, `D081→{D087,D094,D104}`,
> bare-`superseded` withheld), and the **static tree-local protected pathspec** (no external-state
> dependency) — §3; (4) the **KNOWN-SET corpus test** — **zero false positives** on the known
> preserved set (`rfc:0097`/`0027`/`0039`/`0041` + `D174`), known-dead **`decision:D267` nominated**
> — §3; (5) the **read-only candidacy state machine** (`nominated`/`withdrawn`; **no** deletion, **no**
> page, **no** run-admission in P0) — §2/§3; (6) the **read-only-safety regression tests**
> (loop-not-blocked, refresh-not-deferred, no-torn-write, panic isolation) — §4/§7; (7) **resolved
> Open Question 1** with the forward-compatible `(kind,ref)` ON CONFLICT + extensible
> `candidacy_state` ledger schema — §5; and (8) the **§6 P0/P1+ deferral table** — including the two
> re-scoped deferrals **#618** (whole-tree Tier-1 exactness / `status:frozen` citation completeness)
> and **#619** (non-cooperative-FS-hang cull-liveness + late-writer generation fence), plus
> `cull_tombstone`/reaper/soak/`cull_gate`/`accretion_ledger` counterforce/Tiers-2-4. The clearing
> ledger is referenced above and the P0 scope boundary is recorded in **D271**.

---

## §F — Adjudicator-carried findings & build obligations (folded by the committer; weakens no constraint)

The v5 gate cleared **`accept_with_findings`**. The two recorded findings are **non-blocking
accepted P1 deferrals**, each carrying a **downstream P0 build-proof obligation** (the
`binding: false` build-carry constraints `BC-618` / `BC-619`). They are folded here so the
`rfc-0170-p0-build` run carries them forward; **neither reopens nor weakens a cleared constraint**,
and both are subordinate to the verbatim SPEC body below.

**`F5-G1-618` — whole-tree Tier-1 exactness is a documented safe-direction FALSE NEGATIVE
(`posture: tier1_false_positive`, `severity: low`, `status: accepted`).** The static tree-local
protected/frozen manifest protects `docs/records/_frozen/**` but not the tracked `status:frozen`
records outside it (`docs/records/INDEX.md` + `docs/records/audits/*.md`). Under clause 4c those are
**LIVE** sources, so the counted `D081` citations in
`docs/records/audits/STRIATUM_DECISION_RECORD_AUDIT_OPUS_4_8_2026-06-16.md` (hits at `:323` and
`:367-370`, no 4f closure-lexicon token on the matched physical line) conservatively **withhold**
`decision:D081`. `D081` is genuinely dead (`decision-log.md:220` cell 2 `superseded`, cell 5
`Superseded by D087/D094/D104`), so this is an **under-nomination** — the **safe** direction for an
observe-only ledger that nothing acts on — and **never** a live-artifact false positive. Accepted as
the **P1 #618** deferral under the re-scoped bar; **not** a P0 blocker.

- **Build-carry `BC-618` (P0, non-blocking — `binding: false`):** ship the **known-set table-driven
  corpus fixture** asserting **zero false positives** on the known preserved set
  (`rfc:0097`/`0027`/`0039`/`0041` + the still-cited decisions incl. `D174` + `backup/rfc-*` +
  `docs/records/_frozen/**` + RFC-0170 prose), **`decision:D267` nominated**, and **`decision:D081`
  recorded as the documented #618 withheld member** (with the audit citation as its cause) so the
  physical-line 4f filter cannot regress and two implementers read every corpus row identically.
  **P1 #618** then completes whole-tree exactness by extending the static frozen manifest to **all**
  tracked `status:frozen` provenance (protect `docs/records/**`, or add a tree-local clause-4c
  `status:frozen`-non-live rule), keeping `docs/rfcs/` and `docs/decisions/` eligible, so `D081`
  nominates with zero counted hits.

**`F5-G2-619` — the non-cooperative-FS-hang cull-liveness gap is a restart-recoverable READ-ONLY
liveness degradation (`posture: readonly_safety_latency`, `severity: low`, `status: accepted`).**
`cullCtx` bounds **COOPERATIVE** scans (the pgx query cancel + the `fs.WalkDir`/bounded-reader abort
self-terminate within `DefaultCullFoldTimeout`); a **NON-COOPERATIVE** ctx-ignoring blocking syscall
is **not** preemptible by `cullCtx` and holds the single in-flight cull slot until daemon restart
(every later tick logged-skips). Because the stuck worker runs **off the `wait`-gating path** it
never blocks `RunScheduler`, never defers the `scheduler_cursors` refresh, and — never completing its
scan — **never reaches the L4 write phase**, so it produces **no** torn or stale `cullable_entity`
write. Read-only **safety** (recovery never blocked, refresh not deferred, no torn write, panic/error
isolated, no daemon destabilization) is **fully preserved**; only the cull observation stops advancing
until restart. Accepted as the **P1 #619** deferral under the re-scoped bar; **not** a P0 safety break.

- **Build-carry `BC-619` (P0, non-blocking — `binding: false`):** keep the fold **detached and
  O(1)** on the recovery tick with **no** select-on-scan-done join back into the recovery goroutine
  (preserve the off-the-`wait`-path placement that B5's refresh-not-deferred assertion binds on), and
  add the **late-return-zero-write guard test** — a scan that ignores `ctx.Done()` and returns after
  `DefaultCullFoldTimeout` performs **zero** `cullable_entity` UPSERTs (L4 already requires
  completed-before-`cullCtx`-fired before the write phase). **P1 #619** adds cull-slot release on a
  non-cooperative hang + a late-writer generation/epoch fence, proven by a non-cooperative-hang test
  (not one that merely cooperates with `ctx.Done()`).

**§6 deferral table.** The consolidated SPEC's §6 P0/P1+ boundary is **extended** with the **#618**
and **#619** rows above, alongside the previously-named P1+ items (`cull_tombstone`, the reaper, the
soak window, `cull_gate`, the `accretion_ledger` counterforce, Tiers 2–4). P0 builds **only** the
observe-only substrate; none of those land in P0.

---

The consolidated, gate-cleared **v5 SPEC** follows **verbatim** — provenance:
`docs/operator/artifacts/rfc-0170-p0-design-v5/dialogue/holder/HOLDER.md` (`author:
holder-author-001`), reproduced from its re-scoped-bar preamble and §0 onward. Section numbering
(§0–§8) and every `file:line` anchor are the SPEC's own.
