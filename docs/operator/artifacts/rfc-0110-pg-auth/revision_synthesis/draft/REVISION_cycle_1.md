# RFC 0110 — Revision synthesis (cycle 1): cycle-2 constraints discharged, implementation-ready

author: revision-convener-claude-opus-4.8-001
artifact_kind: handoff
logical_name: revision_draft_cycle_1
...
The cycle-2 adjudicator ledger (`COLLABORATION_LEDGER_cycle_2.md`) returned
**`needs_revision`** with **12 binding constraints** (1 critical + 11 high) and
**3 medium follow-ups** (`PX3-005`, `PX3-006`, `OPS-12`). The workflow's
revision budget (`max_iterations: 1`) was exhausted, so the operator recorded
**`dec_b95396ff8f414eae5dbcced5476313ad`** (`accepted_with_follow_up`,
`DECISION_cycle2_proceed.md`): accept the converged cycle-2 design and carry the
cycle-2 findings forward as **binding implementation constraints in the
published spec** rather than cancel the run.

This document is that vehicle. It takes the cycle-2 `constraints[]` as binding
input and **discharges every row explicitly** (§1–§2) with the role's
disposition vocabulary (answer / fold-in / reject-with-rationale /
accept-as-risk / defer-with-successor). It then folds the mediums (§3), states
the normative amendments to `SYNTHESIS_cycle_2.md` (§4), and re-sequences the
L0–L3 plan into release-shaped, gate-first order (§5) with a consolidated
constraint→test matrix (§6). Where this document is silent,
`SYNTHESIS_cycle_2.md` remains normative; where they disagree, **this document
wins**.

Code anchors verified against the current tree at revision time:
`pkg/db/connection.go:207` (pool forces `pgx.QueryExecModeSimpleProtocol`),
`pkg/db/connection.go:82` (generic `BeginTx` on the shared Runner interface),
`pkg/rpc/server.go:119-130` (`auditErr` consulted only to attach `AuditID`; a
failed audit append currently leaves a success response intact — fail-open),
`pkg/db/audit.go:82` (`RecordRPCTransport` opens its own transaction),
`pkg/pgtest/pgtest.go:75-86` (imperative Go `GRANT`/`REVOKE` role layout).
Every cycle-2 implementation finding reproduces against today's source.

---

## 1. Discharge ledger — every cycle-2 `constraints[]` row
...
No row is left open without a recorded disposition; no high/critical row is
rejected. Two `accept-as-risk` half-dispositions are scoped and bounded inside
their fold-ins (§2.3, §2.11), per the decision artifact’s direction.
...
`DefaultQueryExecMode = pgx.QueryExecModeSimpleProtocol` pool-wide (the comment
explains why: multi-statement migration DDL + client-side quoting). Under
simple protocol pgx **interpolates bound parameters into the SQL text**, so the
cycle-2 prelude `SELECT set_config('striatum.daemon_auth', $1, true)` would
serialize the secret into `pg_stat_activity.query`, observable by any
...
to override the default for that single call. The authority/attribution
prelude (and only the prelude) issues its statements as:
...
`ExecBound(ctx, sql, args...)` — which always passes `QueryExecModeExec` and is
the **only** API the prelude uses. All four GUCs (`striatum.daemon_auth`,
`striatum.rpc_id`, `striatum.principal_id`, `app.session_id`) ride `ExecBound`:
the labels are not secrets, but keeping principal/session ids out of
`pg_stat_activity.query` is free here and privacy-positive.
...
custom protocol message, connection-startup payload) — either still text-visible,
stateful across the pool, or native-code adjacent (`C-NO-NATIVE`). Bound
parameters over the extended protocol are the proven secret carrier in
PostgreSQL (it is how password-bearing `pgcrypto` calls are expected to flow).

**Residual (carried from cycle 2, unchanged):** a server operator enabling
verbose statement+parameter logging (`log_parameter_max_length` &c.) can log
bound values. Runbook hardening item; not reachable by a `striatumd_rw`-only
observer.
...
- **G-PRELUDE-MODE** (unit guard): the prelude helper fails a test double whose
  exec mode resolves to `QueryExecModeSimpleProtocol`; asserts `ExecBound` is
  the sole prelude entry point.
...
1. **Type-level:** the write-surface helpers (audit append, artifact append,
   ...
   `MutationTx`, not `TxRunner`. You cannot hand them a generic transaction.
2. **Guard (G-MUTATION-TX):** a repo guard test (same family as the existing
   ...
   migrations, and the recovery sweeps live on an explicit allowlist with
   rationale strings.
3. **SQL-order regression (T-SQL-ORDER):** a recording runner captures the
   statement stream of a compliant handler and asserts
   `set_config('striatum.daemon_auth', …)` is statement 1, before the first
   DML or write-function call; a deliberately non-compliant fake handler fails.
...
exists, as pure plumbing — every mutating handler migrates to it while the
change is still behavior-neutral.
...
- **Mutation-coupled (the binding case).** For mutating RPCs the audit row is
  ...
  success-without-audit-row is impossible by construction, and the audit append
  inherits the transaction’s authority/attribution prelude for free (the same
...
establish. The contract (fail-closed, both modes) is documented in the spec
amendment (§4) — this is the “explicit contract” the constraint demands.
...
failure and assert the response is an error, never success with a missing row.
...
role via a dedicated verb (`striatum daemon owner-ddl apply
--owner-url …`, with the equivalent documented `psql` invocation for operators
who prefer it). Properties:
...
- **Atomic per version.** PostgreSQL DDL is transactional; each bundle version
  ...
  **last, in the same transaction**. A partially-applied bundle therefore
  cannot persist — the “partial owner-DDL state” of the verification matrix is
  structurally unreachable, and the test asserts exactly that (interrupt
  mid-bundle ⇒ no marker, no objects). Bundles exclude non-transactional DDL
...
interrupted-bundle (no marker, no partial objects), and double-apply
(idempotent no-op).
...
imperatively in Go (`CREATE ROLE`, blanket `GRANT SELECT, INSERT, UPDATE,
DELETE`, ad-hoc `REVOKE`s). Security tests against that layout validate the
test harness, not the production migrations — a false-green channel for every
42501 gate.
...
naming (`striatumd_rw_<db>`) survives as a login shell over
migration-defined privileges. pgtest itself is prohibited from issuing
`GRANT`/`REVOKE` on protected-table DML; the §5.1-cycle-2 test-only raw-row
writer remains the sole sanctioned bypass and patches no privileges.
...
Each phase lands with its own negative test (**T-42501-P0/P1/P2**: direct
DML to each protected surface fails `42501`; the authorized path succeeds) and
its own release-note line. The sole-durable-write-path sentence is **reserved**
to P2 in every artifact (spec, D164, RFC, doctor text, README-level claims) —
publishing it earlier is a doc bug with a guard: the spec amendment (§4) keys
the claim to the doctor posture string so they cannot drift apart.
...
7. mixed-format tests (**T-HASH-PARITY**, **T-TS**, **T-VERIFY-MIXED**:
   v2-only, v3-only, mixed-with-continuity, unknown-format, per-format tamper);
...
issue, doctor text, and cycle-2 §7.5’s “structural close of #87” sentence —
is amended to: **“mitigated, pending lane-OS-user default.”** #87 closes only
...
- **Authority lapse is loud.** If the registry row is missing, wiped, or
  ...
  is the re-bootstrap path, consistent with §5.5-cycle-2 fail-closed.
...
roles, both recently rotated) ⇒ no finding; fixture B (two instance ids,
one shared role, recent rotations) ⇒ posture finding. Both assertions in the
...
contract (§2.3), phase nomenclature (§2.7); (c) **RFC 0110** — cycle-2 +
this revision’s deltas. The PR that lands release-N code must be preceded by
(or contain, doc-commits-first) these amendments; G-MUTATION-TX’s guard run
...
daemon-owned table (47 in `striatumd.*` today, from `0001_baseline.sql`
through `0023_principals.sql`), each classified as one of:
...
- `owner_only` — runtime role holds no write privilege
  (`daemon_auth_registry`, `daemon_auth_log`, `schema_authority`,
  `schema_migrations`/`repo_migrations` &c.).
...
| §4.1 (prelude) | Code sketch now issues via `ExecBound` (extended protocol, §2.1) inside `BeginAuthorizedMutation` (§2.2); plain `tx.Exec` prelude is non-conforming. |
| §4.3 (reset) | Destroy-on-any-error → §3.3 bounded policy. |
| §5.2/§7.1 (grants/harness) | pgtest provisioning per §2.5; T-42501/T-GRANT-DRIFT ordering per §2.5. |
...
| §6.1/§6.2 | Unchanged (no cycle-2 constraint touches them). |
...
1. **Release N — authority plumbing, no schema authority:**
   `ExecBound` + extended-protocol prelude (§2.1) · `BeginAuthorizedMutation`
   migration of all mutating handlers + G-MUTATION-TX + T-SQL-ORDER (§2.2) ·
   fail-closed audit contract, mutation-coupled append (§2.3) · capability
   parity checker, inert (§2.11) · bounded-discard reset policy (§3.3) ·
   doctor posture probes (`rotation_skipped_single_role`, `pg_write_boundary`
   reporting `none`) · `daemon_auth_log` writer + redaction.
2. **Release N+1 — owner bundle v1 + L0 + Phase 0 `audit_only`:**
   owner bundle (pgcrypto, registry **with `role_name`**, `daemon_auth_log`,
...
- **Carried from cycle 2 unchanged:** G2 memory-scrape residual (bounded by L0
  ...
  secret (`C-SECRET-HONEST`, RAM-only + rotation-on-restart, not
  zeroization); `V2RowHash` remains permanently as the pre-cutover reader.
