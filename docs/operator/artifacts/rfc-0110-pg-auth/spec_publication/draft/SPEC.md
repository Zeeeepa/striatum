...
logical_name: spec_draft
workflow: rfc-0110-pg-auth-panel
run_id: run_8e14cb48342e929d30043d6be24f9101
status: implementation-ready draft (spec publication)
decision: dec_b95396ff8f414eae5dbcced5476313ad (accepted_with_follow_up)
inputs:
...
## 0. Standing and provenance

This is the publishable RFC/spec for RFC 0110, authored from the **latest
cleared constraint ledger** — not from the original proposal. The authority
chain, binding in this order:

1. **This spec** — the implementation contract. Every binding constraint from
   the cycle-2 adjudication (`COLLABORATION_LEDGER_cycle_2.md`, 12 binding
   constraints: 1 critical + 11 high) lands below as **testable text or a
   named gate**. None is waived.
2. **`REVISION_SYNTHESIS_cycle_1.md`** — the cleared revision this spec
   publishes. Where this spec is silent, it is normative.
3. **`SYNTHESIS_cycle_2.md`** — normative where the revision synthesis is
   silent (v3 hash construction detail, SD-hardening template, privacy gates,
   L2 mechanics, rejected alternatives).

Operator decision `dec_b95396ff8f414eae5dbcced5476313ad`
(`accepted_with_follow_up`) accepted the converged cycle-2 design past the
exhausted revision budget and directed that the cycle-2 findings be carried as
**binding implementation constraints in the published spec**. The discharge
review (`DISCHARGE_REVIEW_cycle_1.md`, `accept_with_findings`) confirmed all
twelve constraints discharged at the synthesis level and named the
carry-forward gates; §14 makes them normative acceptance checks.

Current-source anchors cited here were re-verified at revision time and
spot-checked by the discharge review: `go/pkg/db/connection.go:207` (pool
forces `pgx.QueryExecModeSimpleProtocol`), `go/pkg/db/connection.go:82`
(generic `BeginTx` on the shared `Runner`), `go/pkg/rpc/server.go:119-130`
(audit append fail-open today), `go/pkg/db/audit.go:82` (`RecordRPCTransport`
opens its own transaction), `go/pkg/pgtest/pgtest.go:75-86` (imperative
Go `GRANT`/`REVOKE` role layout).
...
- **L3 — per-RPC attribution.** Every mutating transaction carries
  ...
  prelude — **labels only, never authority** (§4.6).
...
- **Authority negative path (`C-EXEC-AUTH`).** Gate: **T-EXEC-AUTH** — for
  each write fn, calling as `striatumd_rw` with no/wrong
