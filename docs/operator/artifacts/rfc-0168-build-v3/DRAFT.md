...
- F1: UID return is now proof-gated. Scrub cleanup is best-effort but any S1 kill failure, home/provider-store residue, supervisor scratch residue, live non-zombie uid process, or active/abandoned worktree/workspace row owned by the lane UID lease's session keeps the lease quarantined. `retry_quarantined_lane_uids=true` reruns the same proof and returns only on clean P1-P5 proof. Attempt 2 adds focused pgtests for active/abandoned worktree and workspace P5 failures, plus quarantined retry remaining quarantined while dirty and returning only after clean proof.
