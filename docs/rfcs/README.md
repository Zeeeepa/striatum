...
| [0023](0023-web-chat-and-browse.md) | accepted (V1) | Web UI chat + codebase browse: provider-neutral chat client (Anthropic Messages + OpenAI Chat-compatible flavors, configured via env vars), read-only `/browse/<path>` file browser scoped to `--repo`, inline Markdown rendering for `.md` artifacts (closes RFC 0022 V1.5 deferred). Mutation-gated chat startup; CSP-preserving; transcripts stay ephemeral in `.striatum/scratch/` per a narrow D028 carve-out. Adds Markdown library as a runtime dep (recommended `markdown-it-py`). |
