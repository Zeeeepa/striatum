...
| [0025](0025-agent-cli-plugin-bundles.md) | proposed | `striatum plugin install` emits an agent-CLI plugin bundle (Claude Code `.claude-plugin/`, Codex `.codex-plugin/`, Gemini `gemini-extension.json`) wrapping RFC 0015's skill bodies plus five imperative slash commands, an opt-in hooks stub, and a local marketplace fixture. Promotes `gemini` to first-class. Self-contained per D020; offline generation; no hosted services. V1 ships the three first-class profiles in three landable steps. |
