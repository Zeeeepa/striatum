...
| [0036](0036-mcp-harness-for-daemon-v2-mutation-surface.md) | accepted (V1 implemented) | Agent-facing harness for the daemon V2 mutation surface: new `striatum-mcp` skill teaching the preview-then-write idiom, capability/token lifecycle, denial-vocabulary recovery, and capability scope semantics; plus the RFC 0034 §10 chat-assisted scaffolding tool (`generate_workflow_preview` + `generate_workflow_write`) as closed-set chat tools over the RFC 0023 chat surface with operator confirmation enforced before any write. Closes the harness gap left by RFC 0032 V2 + RFC 0034 V1. |
