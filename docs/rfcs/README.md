...
| [0028](0028-long-running-daemon-and-multi-repository-control-plane.md) | proposed | Explore `striatumd`: an optional long-running local daemon that manages multiple target repositories, runs, supervisors, recovery loops, MCP resources/tools, web/CLI clients, and future sealed-provenance authority from one resident control plane. |
