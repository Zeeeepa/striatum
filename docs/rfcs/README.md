...
| [0033](0033-storage-substrate-rewrite-for-daemon-v2.md) | proposed | Pick a non-SQLite substrate for daemon V2 daemon-owned state (resolves RFC 0028 OQ#3 per D086). Recommends embedded PostgreSQL with daemon-owned lifecycle, append-only audit chain enforced by DB roles, and V1 SQLite-registry → V2 PG cutover via `daemon migrate`. Lands first in the daemon V2 follow-up sequence because RFC 0030 keys off the substrate choice. Repo-local `.striatum/state.sqlite3` stays SQLite. |
