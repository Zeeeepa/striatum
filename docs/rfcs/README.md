...
| [0022](0022-web-ui-redesign.md) | proposed | Web UI redesign: server-rendered Jinja2 multi-page (history-API URLs, no hash), refreshed visual palette + dark mode + system fonts + 4px spacing scale, SVG dependency graph with state-colored nodes and click-navigate. Adds Jinja2 as the project's first runtime dependency. RFC 0013 V1's static SPA is superseded; the JSON API and `serve --web` verb stay load-bearing. V1.5 deferred: inline dogfood Markdown rendering. |
