...
| [0038](0038-web-ui-feature-additions-and-frontend-toolchain.md) | proposed | Web UI feature additions over RFC 0037 polish: promote Edit affordance; top-level repo tree browser; `/workflows/new` chooser wizard over RFC 0034 V1 endpoints; drag-drop workflow graph editor with widgets; syntax-highlighted code viewer. Also supersedes D073's implicit "no node toolchain" rule per D092: adopts Vite + React + TypeScript with bundled output shipped in the Python wheel; operator-side install stays pip-only, contributor-side gains an `npm install` step. Islands architecture (Jinja2 page shells + React islands), not full SPA conversion. |
| [0039](0039-go-daemon-core.md) | proposed | Land the Go-language daemon core planned by D084. RFC 0030 envelope-v1 wire protocol and RFC 0033 Postgres substrate stay unchanged — language-agnostic by D084 design. Python and Go daemons coexist via `striatum daemon start --core go` selection during Phase 1; Phase 2/3 retirement of the Python daemon is a future RFC. RFC 0035 multi-repo test harness extends with `daemon_core` parameter to exercise both implementations. Linux + macOS only; Windows daemon stays deferred. CLI stays Python. |
