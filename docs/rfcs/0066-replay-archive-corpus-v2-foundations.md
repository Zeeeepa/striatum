...
bundle verification remains compatible. New V2 manifests also carry a
corpus-scoped incremental export watermark. Any augmentation fetch surface
remains follow-up implementation.
...
metadata, hybrid archive defaults, optional `git_snapshot_hash`, a
corpus-scoped `incremental_export_watermark` with git high-water mark and
dirty-tree advanceability metadata, and verifier support for both implied-V1
and V2 manifests. The archive follow-up adds
...
bounded to any future augmentation-reference fetch surface, which must remain
optional and local.
