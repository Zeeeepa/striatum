...
| **P3 — doctor + refusal + verify** | #347 (133-D) | The generalized barrier doctor invariant (subsumes `fanin_sibling_unintegrated` and the per-integration checks), the `BARRIER_BLOCKED` named state + `blocked_manifest`, `barrier_status` view, `striatum join verify`. | none |
