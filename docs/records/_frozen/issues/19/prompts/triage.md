...
- `docs/issues/21/SPEC.md` — `striatum serve` startup clobbers retired-local-state.
...
- For #21: the minimum is "serve startup must not init-over an existing healthy retired-local-state". A `--read-only` flag and full WAL-mode plumbing are SPEC stretch goals — defer them.
