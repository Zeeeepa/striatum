...
| RFC 0069 | PostgreSQL-only daemon-global surfaces: registry gate, daemon sweep, dashboard-all, daemon MCP resources, owned by the Go port target. | P0 | proposed |
| RFC 0070 | Daemon client/service boundary completion: repo.resolve, `/v1/invoke`, MCP/local API quarantine, dogfood composites. | P1 | proposed |
| RFC 0071 | Operator diagnostics and cutover evidence: migration cleanup report, authority doctor, generated matrix follow-up. | P2 | proposed |
