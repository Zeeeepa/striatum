...
| `operator.bootstrap` | n/a (MCP/RPC; `operator bootstrap` CLI client rewire is a P0 follow-up) | admin | single_repo | n/a (RFC 0167 P0, Go-only) | real | no | no | RFC 0167 P0 (#D260) mints a session-bound operator token ({admin, read}, repo-scoped) + leases a handle in one transaction (resolve/create principal, link client, INSERT operator_sessions, acquire lease); presents the session-bound operator token (§F F-2: the static bootstrap token is not injected for routine repo-admin) |
