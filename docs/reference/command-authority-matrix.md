...
| `daemon_under_load` | The operation timed out behind transient daemon back-pressure (a statement_timeout/57014 event-append convoy under multi-run supervise load) rather than a real refusal, after the daemon already retried it (#198/#355). | Retry the operation shortly; if it persists, check daemon PostgreSQL load with `striatum doctor` and look for a long-held lock on repo_event_chain_heads. |
