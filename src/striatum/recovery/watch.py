"""RFC 0020 step 3: long-lived ``recovery watch`` daemon.

Wraps :func:`striatum.recovery.run_auto_sweep` in a sleep loop with
single-instance pidfile + signal-driven shutdown + JSONL emission.

The daemon is *purely* operational: all state mutation flows through
``run_auto_sweep`` (which routes through CLI verbs internally — D006/
D009). The watcher contributes scheduling, signal handling, and
output formatting, nothing else.
"""
...
def run_watch(
    ...
    *,
    ...
    if not acquired:
        ...
        _emit_line(out, json_output, {"event": "watch_collision",
                                       "run_id": run_id,
                                       "pid": existing,
                                       "message": message},
    ...
    try:
        while not stop_event.is_set():
            ...
            if exit_on_terminal:
                ...
                if state_row is not None and str(state_row["state"]) in TERMINAL_RUN_STATES:
