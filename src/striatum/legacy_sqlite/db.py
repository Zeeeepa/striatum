...
def connect(repo: Path) -> sqlite3.Connection:
    ...
    :func:`striatum.legacy_sqlite.migrations.apply_migrations` on every
    connect, so upgrades are silent and automatic. Connecting to a database
    whose schema is newer than this install supports raises a clear
    ``StriatumError``.
...
def _refuse_init_when_install_lags_repo(repo: Path) -> None:
    ...
    from striatum.legacy_sqlite.migrations import LATEST_VERSION as install_latest
