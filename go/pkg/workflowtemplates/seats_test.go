...
func TestSeatTierForAdapterAgySupported(t *testing.T) {
	// agy is backed by the RFC 0109 two-turn installed-CLI seat gate.
...
func TestSupportedSeats(t *testing.T) {
	// Supported seats must stay in lockstep with adapterconformance's installed-CLI
	// fixture registry.
	got := SupportedSeatAdapters()
	want := []string{"agy", "codex"}
	if strings.Join(got, ",") != strings.Join(want, ",") {
		t.Fatalf("SupportedSeatAdapters() = %v, want %v", got, want)
	}
}
