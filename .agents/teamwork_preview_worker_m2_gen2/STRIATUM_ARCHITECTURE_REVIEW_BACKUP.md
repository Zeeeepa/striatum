...
*   **Centralized State Store**: Pursuant to D094 / RFC 0043, SQLite and repo-local state files are retired. All workflow state (runs, sessions, leases, and jobs) resides in a daemon-owned PostgreSQL database, ensuring strict transactional boundaries.
...
*   **Cryptographic Append-Only History**: Database tables holding the core events and artifacts are locked down using native PL/pgSQL triggers and database-role permissions (`striatumd_rw`) that revoke `UPDATE` and `DELETE` access.
...
*   **Materialized Projections**: Introspective operations (such as the CLI status dashboard or execution trajectory views) are materialized projections computed on-demand from an append-only transaction ledger rather than mutable state columns.
...
1.  **The CLI (`striatum`)**: Exposes commands to the user. It either runs purely local, stateless operations (like `workflow validate` or `skills install`) or dispatches JSON-RPC packets over Unix sockets to the daemon.
...
*   **Stated**: Access to daemon mutations is locked down per capability (`read`, `write`, `claim`, `review`, `apply`, `admin`, `recovery`) and scoped strictly to individual repositories, as specified in `docs/reference/command-authority-matrix.md` and `docs/how-to/postgres-transition.md`.
*   **Actual**: In `go/pkg/rpc/auth_pg.go:43-110`, the `PostgresAuthorizer` performs a database-backed capability check. It extracts the `token_id`, queries `striatumd.clients` (lines 61-66) to retrieve token hashes and salts, and performs a constant-time HMAC check using `subtle.ConstantTimeCompare` (lines 73-76). It then queries `striatumd.client_capabilities` (lines 88-101) to verify the capability matches the `repository_id` scope.
*   **Mine**: Using database-driven client registration with HMAC token matching and constant-time execution is production-grade security architecture. However, querying the database synchronously on *every* RPC transaction creates massive serialization overhead. For a fast-moving terminal agent executing dozens of check/read/write loops, the overhead of repeating these Postgres select statements is an unnecessary bottleneck. The system should utilize a thread-safe in-memory cache within the daemon process (e.g. a token-to-capability registry with a short 5-second TTL) to alleviate SQL query pressure.

#### C. Model Context Protocol Loopback Guardrails
*   **Stated**: The Model Context Protocol (MCP) server must expose database workflow mutations as actionable tools directly to AI terminal agents while maintaining loopback isolation and preventing remote exploits. This constraint is documented in `docs/reference/spec.md` under "Model Context Protocol Security" and `AGENTS.md`.
*   **Actual**: In `go/pkg/mcp/http.go:541-550`, the helper function `validateLocalRequest` acts as an absolute gatekeeper:
    ```go
    ...
    ```
    If these assertions fail, the HTTP endpoint returns a **403 Forbidden** status immediately. Furthermore, `go/pkg/mcp/capabilities.go:60-74` hides all workflow authoring tools, and `go/pkg/mcp/tools.go:34-37` intercepts attempts to call them, failing closed with a `tool_hidden` error.
*   **Mine**: Host and Origin header loopback validation is an outstanding local-first security guardrail. Local developer tools that run HTTP servers on arbitrary ports are highly vulnerable to Drive-By exploits—where a malicious public website executes local curl payloads in the background using the user's browser. Striatum's check is robust and elegant. My only criticism is the lack of randomized port assignment. The daemon binds to a static loopback port; if a local attacker spins up a process on that port first, they can block the daemon. Exposing the active port dynamically via an encrypted local file inside `.striatum/` would make this system bulletproof.

#### D. Attested Process Lane Supervision
*   **Stated**: Process supervision must securely assert that the active process executing a job is indeed the registered agent shell, and not a recycled PID or an operator override. Attested bylines are required for committed Markdown artifacts, as specified in `AGENTS.md` and `docs/explanation/domain-driven-design.md`.
*   **Actual**: In `go/pkg/supervisor/process_identity_linux.go:13-32`, the system calls `ProcessStartToken(pid)` to read `/proc/<pid>/stat`. It extracts field 22 (ticks since boot) as an immutable start-time token. During lane liveness validation (e.g. `go/pkg/supervisor/tmux_liveness.go:141-206` or `go/pkg/mutations/claim.go:705-727`), the daemon verifies this start token matches the active db record. If matched, the byline is derived as `author: <role>-<model>-<ordinal>`; otherwise, it falls back to a non-attested `author: operator` string.
*   **Mine**: Coupling Unix PIDs with boot-tick start-times from Linux `procfs` is a brilliant, highly sophisticated mechanism to solve the PID recycling vulnerability. Unix PIDs are volatile and recycle rapidly; relying on PID numbers alone is a classic TOCTOU (Time-of-Check to Time-of-Use) security defect. The start-time tick acts as an signature of the process instance. However, this implementation is hardcoded for Linux. When running on macOS, this check falls back to un-attested status due to the absence of `/proc`. The Darwin implementation should be enhanced to leverage `proc_pidinfo` syscalls to maintain attestation parity.

#### E. Strict Database Migrations drift-protection
*   **Stated**: SQLite states have been retired completely, and database schemas are progressively migrated using transaction-safe, advisory-locked Go embedded migrations, as defined in `docs/how-to/postgres-transition.md`.
*   **Actual**: In `go/pkg/db/migrations.go:17-18`, the system registers `LatestDaemonDBVersion = 17` and a hardcoded advisory lock key `332933`. In `VerifyMigrationsSHASource` (lines 159-195), it computes SHA256 hashes of compiled embedded DDL SQL files and compares them against physical files on disk, failing the daemon bootstrap if drift is detected.
*   **Mine**: Drift protection via SHA256 checksum mapping is a phenomenal dev-ops safety check. It prevents developers from editing migration files post-release. However, the migration advisory lock implementation is highly problematic. Using a single hardcoded 32-bit key `332933` means that if multiple repositories are registered on the same local Postgres server, or if separate daemon instances are booted, they will share this lock key. On startup, a second daemon instance will block indefinitely waiting for the lock—even if they operate on entirely separate databases. The advisory lock key must be derived dynamically by hashing the database name or the schema scope.
...
*   **Actual**: In `go/pkg/admin/repo_init.go:314-328`, the adoption routines create `.striatum/` and `.striatum/scratch/` using owner-only permissions (`0o700`). In `ensureGitignore` (lines 330-346), it automatically appends `.striatum/` to the repo's `.gitignore` file. In `go/pkg/mutations/supervision_control.go:125-134`, the daemon creates Named Pipes (`stdin.pipe`) at `.striatum/scratch/<supervisor_id>/stdin.pipe` using `syscall.Mkfifo` to inject JSON work packets directly into active supervisor shells.
*   **Mine**: Clean architectural decoupling between state (Postgres) and transient IO mechanisms (FIFO pipes/PTYs inside `.striatum/`) is highly elegant. Using 0o700 permissions shields active named pipes from local snooping. However, in Unix, FIFO pipes can block indefinitely on open/write calls if no reader is attached. In `supervision_control.go:256-300`, the system opens the FIFO non-blockingly using `O_WRONLY|O_NONBLOCK` to prevent daemon freezes. While correct, this results in silent packet drops (ENXIO) if the helper process detaches or restarts. The daemon should maintain a persistent, bounded ring-buffer in memory to hold the last three unacknowledged packets for a reattaching supervisor.
...
### C. SERIOUS: Blocking FIFO Writes on Supervision Degraded States (ENXIO)
*   **Location**: `go/pkg/mutations/supervision_control.go:275-290` (HandleSuperviseSend FIFO write).
*   **Stated**: Packets are delivered non-blockingly to active processes via Named Pipes.
*   **Actual**: The write operation is performed non-blockingly:
    ```go
    fd, err := os.OpenFile(pipePath, os.O_WRONLY|os.O_NONBLOCK, 0)
    if err != nil {
        if os.IsNotExist(err) || errors.Is(err, syscall.ENXIO) {
            // Degraded path - reader has exited
        }
    }
    ```
*   **Mine**: In Linux, opening a FIFO pipe with `O_WRONLY|O_NONBLOCK` immediately returns `ENXIO` if no process is currently reading from the other end of the pipe. When a supervisor detached state occurs (e.g., tmux pane detached, helper process restarting, or transient IO lag), this call fails instantly. Any instruction packet intended for the agent is permanently dropped by the daemon, which then marks the lane degraded. The agent shell is left in a hung or waiting state, requiring manual operator intervention. The daemon must maintain a persistent, bounded ring-buffer in memory to queue transit packets during temporary connection losses, allowing the supervisor helper to drain them immediately upon reattachment.

---

### D. SMELL: Synchronous SQL Queries on Capabilities Authorizer
