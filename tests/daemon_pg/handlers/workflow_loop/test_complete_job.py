...
from striatum.daemon_pg.handlers.workflow_loop.complete_job import handle
...
def test_complete_job_releases_work_enqueues_downstream_and_preserves_repo_scope(
    tmp_path: Path, pg_url: str
) -> None:
    conn = connect(pg_url)
    try:
        _seed_running_job(conn, tmp_path, repository_id="repo_a", with_downstream=True)
        _seed_running_job(conn, tmp_path, repository_id="repo_b", with_downstream=False)
        ...
        assert result == {"status": "completed", "job_id": "job_1"}
        assert _states(conn, repository_id="repo_a", job_id="job_1", message_id="msg_1") == {
            "job": "completed",
            "job_lease": None,
            "message": "completed",
            "message_lease": None,
        ...
        assert _one(
            ...
            "WHERE repository_id = %s AND job_id = %s AND state = 'pending'",
            ("repo_a", "job_2"),
        ) == {"count": 1}
        assert _states(conn, repository_id="repo_b", job_id="job_1", message_id="msg_1") == {
            "job": "running",
            "job_lease": "lease_1",
            "message": "acked",
        ...
        assert events[1]["payload_json"] == {"summary": "done"}
        seed_hash = canonical_event_hash(events[0])
        completed_hash = canonical_event_hash(events[1], previous_hash=seed_hash)
        enqueued_hash = canonical_event_hash(events[2], previous_hash=completed_hash)
        assert completed_hash != seed_hash
        assert enqueued_hash != completed_hash
    finally:
        conn.close()


def test_complete_job_completes_run_when_last_nonterminal_job_finishes(
    tmp_path: Path, pg_url: str
) -> None:
    conn = connect(pg_url)
    try:
        _seed_running_job(conn, tmp_path, repository_id="repo_a", with_downstream=False)
        conn.commit()
        ...
        assert _one(
            conn,
            "SELECT state, stop_reason FROM striatumd.runs WHERE repository_id = %s AND run_id = %s",
            ("repo_a", "run_1"),
...
def _seed_running_job(
    ...
    workflow = {
        ...
        "lanes": {"local": {}},
        "jobs": workflow_jobs if with_downstream else workflow_jobs[:1],
    }
...
def _states(conn: Any, *, repository_id: str, job_id: str, message_id: str) -> dict[str, Any]:
    job = _one(
        conn,
