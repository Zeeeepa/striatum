...
from striatum.daemon_pg.handlers.workflow_loop.release_lease import handle
...
def test_release_requeues_review_only_work_and_preserves_other_repository(
    tmp_path: Path, pg_url: str
) -> None:
    conn = connect(pg_url)
    try:
        _seed_claimed_work(conn, tmp_path, repository_id="repo_a", repo_write=False)
        _seed_claimed_work(conn, tmp_path, repository_id="repo_b", repo_write=False)
        ...
        assert result == {"status": "released", "job_state": "queued"}
        ...
        assert [row["event_type"] for row in events] == ["seed.event", "lease.released"]
        assert events[-1]["event_id"] > seed_event_id
        assert events[-1]["payload_json"] == {"reason": "yielding", "job_state": "queued"}
        seed_hash = canonical_event_hash(events[0])
        release_hash = canonical_event_hash(events[-1], previous_hash=seed_hash)
        assert release_hash != seed_hash
    finally:
        conn.close()


def test_release_blocks_repo_write_work_even_when_requeue_requested(
    tmp_path: Path, pg_url: str
) -> None:
    conn = connect(pg_url)
    try:
        _seed_claimed_work(conn, tmp_path, repository_id="repo_a", repo_write=True)
        conn.commit()
        ...
        result = handle(
            _ctx(conn, tmp_path, repository_id="repo_a"),
            {
                ...
                "message_id": "msg_1",
                "lease_id": "lease_1",
                "reason": "cannot requeue repo write",
                "requeue": True,
            },
        )

        assert result == {"status": "released", "job_state": "blocked"}
        assert _states(conn, repository_id="repo_a")["job"] == "blocked"
        assert _states(conn, repository_id="repo_a")["message"] == "blocked"
    finally:
        conn.close()
...
def _seed_claimed_work(
    conn: Any, tmp_path: Path, *, repository_id: str, repo_write: bool
) -> None:
    ...
    with conn.cursor() as cur:
        ...
        cur.execute(
            ...
            """,
            (repository_id, Jsonb(["claim"]), now, now),
        )
        cur.execute(
            """
        ...
        cur.execute(
            ...
            VALUES (%s, 'msg_1', 'run_1', 'job_1', 'work', 'acked', 'reviewer',
                    %s, %s, %s, %s, %s, 'lease_1')
            """,
