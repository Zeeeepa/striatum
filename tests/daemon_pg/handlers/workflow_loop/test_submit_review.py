...
from striatum.daemon_pg.handlers.workflow_loop.submit_review import handle
...
def test_submit_review_acks_claimed_job_publishes_artifact_and_records_verdict(
    tmp_path: Path, pg_url: str
) -> None:
    conn = connect(pg_url)
    try:
        repo_root = tmp_path / "repo_a"
        review_path = repo_root / "docs" / "review.md"
        review_path.parent.mkdir(parents=True)
        review_path.write_text("# Review\nauthor: operator\n\nLooks good.\n", encoding="utf-8")
        _seed_claimed_review(conn, repo_root, repository_id="repo_a")
        conn.commit()
        ...
        assert result["artifact"]["status"] == "published"
        assert result["verdict"]["verdict"] == "accept"
        artifact_id = str(result["artifact"]["artifact_id"])
        assert _one(
            conn,
            "SELECT artifact_id, repo_path, author_line FROM striatumd.artifacts "
        ...
        assert _one(
            conn,
            "SELECT findings_artifact_id, verdict FROM striatumd.verdicts "
            "WHERE repository_id = %s AND job_id = %s",
            ("repo_a", "job_review"),
        ) == {"findings_artifact_id": artifact_id, "verdict": "accept"}
...
def _seed_claimed_review(conn: Any, repo_root: Path, *, repository_id: str) -> None:
    ...
    expected = [
        {"logical_name": "review", "kind": "finding", "path": "docs/review.md", "required": True}
    ]
    with conn.cursor() as cur:
        ...
        cur.execute(
            ...
            INSERT INTO striatumd.jobs (
              ...
              created_at, current_message_id, current_lease_id
            )
            VALUES (%s, 'job_review', 'run_1', 'review', 'Review', 'review',
                    'reviewer', %s, %s, 'claimed', %s, %s, 'review-1',
                    %s, 'msg_1', 'lease_1')
            """,
            (
                repository_id,
                Jsonb({"lane_id": "local"}),
                Jsonb({}),
                Jsonb({"repo_write": False, "allowed_paths": ["docs/"], "forbidden_paths": []}),
                Jsonb(expected),
                now,
            ),
        )
        cur.execute(
            ...
            VALUES (%s, 'msg_1', 'run_1', 'job_review', 'work', 'claimed',
                    'reviewer', %s, %s, %s, %s, 'lease_1')
            """,
            (repository_id, Jsonb({}), now, now, now),
        )
        cur.execute(
            """
