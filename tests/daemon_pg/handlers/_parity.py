"""Shared parity helpers for RFC 0048 PG vs SQLite handler tests.
...
handler and the SQLite CLI helper for the same input fixture. This
...
def assert_payload_parity(
    ...
    *,
    ...
    sqlite_result
        Payload returned by the SQLite-backed handler.
    ignore_keys
    ...
    if diffs:
        raise AssertionError(
            "PG vs SQLite handler payload parity diff:\n"
...
def _diff(left: Any, right: Any, path: str, diffs: list[str]) -> None:
    ...
    if isinstance(left, dict):
        ...
        for missing in sorted(left_keys - right_keys):
            diffs.append(f"{path}.{missing}: present in PG, missing in SQLite")
        for extra in sorted(right_keys - left_keys):
            diffs.append(f"{path}.{extra}: missing in PG, present in SQLite")
